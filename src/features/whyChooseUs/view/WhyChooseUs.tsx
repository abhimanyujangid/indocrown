...
export default function WhyChooseUs({ dict }: WhyChooseUsProps) {
  ...
  return (
    <Box ref={containerRef} component="section" id="why-choose-us" sx={{ height: '300vh', bgcolor: 'common.white', position: 'relative' }}>
      ...
      <Box sx={{ position: 'sticky', top: 0, height: '100vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', pt: { xs: 12, md: 16 } }}>
        <Container maxWidth="md" sx={{ textAlign: 'center', mb: 8, px: { xs: 2, sm: 3, md: 4 } }}>
          ...
          <Typography variant="h2" sx={{ maxWidth: 800, mx: 'auto' }}>
            {section.headline}
          </Typography>
        </Container>

        {/* Cards Wrapper */}
        <Box sx={{ position: 'relative', flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'flex-start', mt: 4 }}>
          
          {/* Card 0: Left (Dark) */}
          <Box
            component={motion.div}
            style={{ x: leftX, rotate: leftRotate }}
            sx={{ position: 'absolute', width: { xs: 280, sm: 320, md: 360 }, height: 420, bgcolor: '#0C1009', color: 'common.white', borderRadius: 4, p: 4, boxShadow: '0 20px 40px rgba(0,0,0,0.1)', zIndex: 1, transformOrigin: 'bottom right' }}
          >
            <Box sx={{ width: 'full', height: 220, borderRadius: 2, overflow: 'hidden', mb: 4 }}>
              <img src={section.cards[0].image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </Box>
            <Typography variant="h5" sx={{ mb: 2, fontWeight: 600 }}>{section.cards[0].title}</Typography>
          ...
          <Box
            ...
            sx={{ position: 'absolute', width: { xs: 280, sm: 320, md: 360 }, height: 420, bgcolor: 'primary.main', color: 'common.white', borderRadius: 4, p: 4, boxShadow: '0 20px 40px rgba(0,0,0,0.1)', zIndex: 1, transformOrigin: 'bottom left' }}
          >
          <Box sx={{ width: 'full', height: 220, borderRadius: 2, overflow: 'hidden', mb: 4 }}>
              <img src={section.cards[2].image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </Box>
            <Typography variant="h5" sx={{ mb: 2, fontWeight: 600 }}>{section.cards[2].title}</Typography>
          ...
          <Box
            component={motion.div}
            sx={{ position: 'absolute', width: { xs: 280, sm: 320, md: 360 }, height: 420, bgcolor: '#F8F9FA', color: 'text.primary', borderRadius: 4, p: 4, boxShadow: '0 20px 60px rgba(0,0,0,0.15)', zIndex: 2 }}
          >
            <Box sx={{ width: 'full', height: 220, borderRadius: 2, overflow: 'hidden', mb: 4 }}>
              <img src={section.cards[1].image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            </Box>
            <Typography variant="h5" sx={{ mb: 2, fontWeight: 600 }}>{section.cards[1].title}</Typography>
